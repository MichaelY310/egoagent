...
def create_harness(harness_dir: str, agents: str, prompts: str = None, workspace: str = None, inherit_conversation: bool = False, return_mode: str = None):
    ...
    session_info = f"harness={harness.name}, slots=[{slots_info}], workspace={harness.workspace or 'None'}, session={harness.session.save_dir}"

    summary_parts = [f"=== 子 session 内容 ({session_info}) ===\n"]
    ...
    summary_parts.append(f"\n=== 以上是子 session ({session_info}) 的全部内容。子 session 已结束，根据以上内容继续之前的任务。 ===")
    return "\n".join(summary_parts)
